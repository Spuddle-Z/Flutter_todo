...
class CalendarController extends GetxController {
  ...
  void updateViewMonth(int offset) {
    viewMonth.value = DateTime(viewMonth.value.year, viewMonth.value.month + offset);
    firstDayIndex.value = DateTime(viewMonth.value.year, viewMonth.value.month, 1).weekday % 7;
    viewMonthString.value = '${viewMonth.value.year} - ${months[viewMonth.value.month - 1]}';
  }
  ...
  DateTime getCellDate(int index) {
    if (index < firstDayIndex.value) {
      final DateTime lastMonth = DateTime(viewMonth.value.year, viewMonth.value.month, 0);
      return DateTime(
        lastMonth.year,
        lastMonth.month,
        lastMonth.day - firstDayIndex.value + index + 1
      );
    } else if (index < firstDayIndex.value + DateTime(viewMonth.value.year, viewMonth.value.month+1, 0).day) {
      return DateTime(
      ...
      return DateTime(
        ...
        index - firstDayIndex.value - DateTime(viewMonth.value.year, viewMonth.value.month+1, 0).day + 1
      );
    }
  }
}
